...
template <typename F>
void QueryTwoDTree(VecView<PolyVert> points, manifold::Rect r, F f) {
  if (points.size() <= 8) {
    for (const auto& p : points)
      if (r.Contains(p.pos)) f(p);
    return;
  }
  manifold::Rect current;
  ...
  std::array<manifold::Rect, 64> rectStack;
  ...
  while (1) {
    ...
    manifold::Rect left = current;
    manifold::Rect right = current;
