...
class EarClip {
 ...
 private:
  ...
  std::map<VertItr, manifold::Rect> hole2BBox_;
  ...
  struct Vert {
    ...
    double EarCost(double epsilon, IdxCollider& collider) const {
      ...
      manifold::Rect earBox = manifold::Rect(vec2(center.x - radius, center.y - radius),
  ...
  void CutKeyhole(const VertItr start) {
    const manifold::Rect bBox = hole2BBox_[start];
