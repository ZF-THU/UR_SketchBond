...
bool FFromLZSessionReset::HandleGlobalTab(UWorld* World)
{
	...
	if (GFromLZActiveCompositeTaskCount.Load() > 0)
	{
		UE_LOG(LogTemp, Log, TEXT("FromLZ reset: Tab pressed; waiting for %d active composite task(s) to finish before resetting."), GFromLZActiveCompositeTaskCount.Load());
	}
	else
	{
		UE_LOG(LogTemp, Log, TEXT("FromLZ reset: Tab pressed; reset will finalize on the next tick."));
	}
